...
describe("a memory parser", () => {
  ...
  const tests = [
    ...
    ["parses strings w/ fractional amounts", "6.5E", 6.5 * 1000 ** 6],
    ["parses strings w/ leading decimals", "-.61Ti", -0.61 * 1024 ** 4],
    ["parses strings w/ trailing decimals", "-69.T", -69 * 1000 ** 4],
    ["parses strings w/ leading plus sign", "+7.45Ki", 7.45 * 1024 ** 1],
  ];
  ...
  it("should throw on invalid strings", () => {
    expect(() => {
      memoryParser("hello");
    }).toThrowErrorMatchingInlineSnapshot(`"Invalid quanity provided."`);
  });
});
